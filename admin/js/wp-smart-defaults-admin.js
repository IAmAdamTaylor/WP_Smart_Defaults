( function( $, window, document, undefined ) {
	...
	function update_logo_id( id ) {
		var $input = $( '#wpsmartdefaults_logo_id' );
		
		$input.val( id );
	}

	function show_logo_preview( src ) {
		var $image = $( '#wpsmartdefaults_upload_logo_preview' );
	...
	function update_favicon_id( id ) {
		var $input = $( '#wpsmartdefaults_favicon_id' );
		
		$input.val( id );
	}

	function show_favicon_preview( src ) {
		var $image = $( '#wpsmartdefaults_upload_favicon_preview' );
	...
	$('.js-wpsmartdefaults-open-upload-frame').on( 'click', function( event ) {
		...
		file_frame.on( 'select', function() {
			...
			if ( 'wpsmartdefaults_upload_logo_button' == $this.attr('id') ) {

				update_logo_id( attachment.id );
				show_logo_preview( attachment.url );

			} else if ( 'wpsmartdefaults_upload_favicon_button' == $this.attr('id') ) {
  ...
  $('#wpsmartdefaults_logo_width').on( 'change', function() {
  	var $this = $(this),
  			width = $this.val();
  	...
  	$('#wpsmartdefaults_upload_logo_preview').css( {
  		width: width
  	} );
  } );
